...
import { CheckoutPaymentStep } from '@/components/store/checkout-payment-step';
...
async function getPaymentMethods(subdomain: string) {
  ...
  if (!res.ok) return [];
  const data = await res.json();
  return (data.methods ?? []) as Array<{ type: string; enabled: boolean; config?: Record<string, string> }>;
}
...
export default async function CheckoutPaymentPage({
  ...
  searchParams: { orderId?: string; order?: string; method?: string };
}) {
  const { subdomain } = params;
  const orderId = searchParams.orderId ?? '';
  const orderNumber = searchParams.order ?? '';
  if (!orderId || !orderNumber) notFound();
  ...
  return (
    ...
    >
      ...
      <CheckoutPaymentStep
        ...
        initialMethod={searchParams.method}
      />
    </div>
  );
}
